...
impl InnerSIAndroidCom {
    pub async fn new() -> Result<Self, SiacomError> {
        ...
        let fd = tokio::task::spawn_blocking(move || -> io::Result<i32> {
            ...
            let jvm = unsafe { JavaVM::from_raw(_jvm_ptr as *mut _) }
            ...
            let package_name: String = env
                .get_string(&JString::from(_package_name))
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?
                .into();

            info!("Getting Context.USB_SERVICE");
            let _usb_service_str = env
                .get_static_field(
            ...
            while env
                .call_method(&iterator, "hasNext", jni_signature(None, "boolean"), &[])
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?
                .z()
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?
            {
                let device = env
                    .call_method(
                        &iterator,
                        "next",
                        jni_signature(None, "java.lang.Object"),
                        &[],
                    )
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?
                    .l()
                ...
                let device_vid = env
                    .call_method(&device, "getVendorId", jni_signature(None, "int"), &[])
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?
                    .i()
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?;

                let device_pid = env
                    .call_method(&device, "getProductId", jni_signature(None, "int"), &[])
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?
                    .i()
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?;
            ...
            let jdevice = _dev.ok_or(io::Error::new(
                io::ErrorKind::Other,
                format!("SI Device not found"),
            ))?;
            ...
            let p_action_str = env
                .new_string(format!("{}.USB_PERMISSION", package_name))
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?;
            info!("Creating Intent object");
            let p_intent_obj = env
                .new_object(
                    "android/content/Intent",
                    jni_signature(Some(vec!["java.lang.String"]), "void"),
                    &[JValue::from(&p_action_str)],
                )
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?;

            info!("getting intent - getBroadcast");
            let p_pending_intent = env
                .call_static_method(
            ...
            let device_conn = env
                .call_method(
                    ...
                    "openDevice",
                    jni_signature(
                        Some(vec!["android.hardware.usb.UsbDevice"]),
            ...
            if device_conn.is_null() {
                ...
                return Err(io::Error::new(
                    io::ErrorKind::Other,
                    format!("Failed to open device connection"),
                ));
            }
